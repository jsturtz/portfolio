...
/*--------------------------------------------------------------------------------------------
INPUT PARTITION 
Input case 2: Deck has less than three cards
...
int main() {
  ...
  printf("TESTING FUNCTION \"cardEffect\" for card Smithy\n");

  // Input case 2: Deck has only two cards left
  printf("Input Case 2: Deck has only two cards left\n");
    
  setDeck(&G, player, 2);
  setHand(&G, player, handpos);                         // sets hand to ensure errors from initialization do not affect
  memcpy(&testG, &G, sizeof(struct gameState));         
  cardEffect(smithy, 0, 0, 0, &testG, handpos, &coin_bonus); // the 0s are booleans for choice 0, 1, & 2, not relevant for adventurer

  // runs the same tests for all inputs because these tests do not depend on particular input 
  runTests(player, handpos, &G, &testG);

  // TEST: The top two cards in deck should be added to hand + some unknown from shuffled discard
  pass = 1;
  for (int i = 0; i < 2; i++) {
    nextCard = G.deck[player][G.deckCount[player]-i];
    if (nextCard != testG.hand[player][G.handCount[player]+i]) {
      pass = 0; 
      break;
    }
  }

  if (pass) printf("\tPASSED: The two remaining cards on deck were added to hand\n");
  else printf("\t\u274CFAILED: Expected two cards from hand to match remaining two cards in deck\n");

  // TEST: DiscardCount should be 0
