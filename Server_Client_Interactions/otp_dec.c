...
{
  int socketFD, portNumber, charsSent, charsRead, charsReadInChunk, charsLeft;
  ...
  do
  {
    charsReadInChunk = recv(socketFD, buffer + charsRead, charsLeft, 0); // Read the client's message from the socket
  ...
  } while (charsLeft > 0); // needed because not all info may be sent at once

  // just going to arbitrarily use 1 as the character indicating incorrect connection to otp_dec_d
  if (buffer[0] == '1') 
  {
    fprintf(stderr, "ERROR: otp_dec cannot connect to otp_enc_d, port %d\n", portNumber);
    exit(2);
  }
